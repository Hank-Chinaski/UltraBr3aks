...
## Second matched Native run: September 12 at 05:12

**Confirmed fact, user-reported:** Bryan repeated the same mode, teams and quarter length, without shortening quarters. This time the opponent inbounded. Around six seconds into the second quarter there was a three-to-four-second pause and recovery, followed by a permanent video/audio freeze around twelve seconds into the quarter. Bryan forced a restart and preserved `C:\Users\PIZZAOVEN\Desktop\Mem\Native-20260912-0x154_02.dmp`.

**Confirmed fact, direct dump comparison:** The new dump's internal time is **05:12:02.675 PDT**. It independently repeats `0x154`, `nt!RtlDecompressBufferLz4+0xb0`, original exception `c0000006`, lower status `c000000e`, and the same failure bucket/hash as the previous Native run. Native rendering remains selected. The owning thread is now FormlabsCrashHandler.exe attached to MemCompression, versus svchost.exe previously; this identifies the immediate memory consumer, not a new software culprit.

NBA's elapsed process time at the two crashes was **8:22.942** and **7:52.084**, respectively. Two failures now support the early-second-quarter phase as a specific reproduction target. Bryan's own inbound pass and the exact clock-start instant are unnecessary for the latest recurrence. Because quarter length was unchanged and elapsed runtimes are also close, phase and time/accumulated activity have not yet been separated.

**Subsequent clarification, confirmed user report:** Both runs were **MyNBA with 5-minute played quarters**. The proposed single comparison is the same MyNBA matchup with 1-minute played quarters if offered, otherwise the shortest supported duration below 5, retaining the other current settings. Observe the recap and second quarter, stop at the first crash, and restore 5 minutes after the comparison. The exact NBA 2K27 minimum/menu path has not been verified; no menu-path claim is made. Passing the second quarter would not by itself establish a fix. This test is proposed and has not been run by the assistant.

The captured NBA threads and three outstanding IRPs did not identify the initiating operation. The dump again lacks translation data needed to map the failed compressed-memory source page to an exact paging slot/device request. A System-log query for 05:10:30-05:12:10 returned no matching events, so no reset/retry explanation is established for the brief recovery. Full details and evidence are in [the second Native run analysis](native-crash-20260912-02/FINDINGS.md). No additional system or game change was made.

## Third Native run: one-minute quarters and direct C: controller evidence

**Confirmed fact, user report and dump:** Bryan changed only MyNBA played-quarter length from 5 minutes to 1 minute, retaining the same game/teams. The crash occurred with 5 seconds remaining in Q2. New preserved copy: `C:\Users\PIZZAOVEN\Desktop\Mem\Native-20260912-03.dmp`. Internal bugcheck time **10:26:17.507 PDT**, NBA elapsed **4:06.920**, again `0x154`, `RtlDecompressBufferLz4+0xb0`, original `c0000006` and lower `c000000e`, same failure bucket/hash. The immediate owning process is now CastSrv.exe attached to MemCompression; it is not designated the cause.

**Major new confirmed evidence:** Storage structures identify C:'s Samsung 990 PRO serial ending **3141_8E54** as the boot/paging NVMe path. Its last 1,000 retained Storport entries include **388 NoDevice read/write completions**, corresponding build-I/O entries and **zero HwStorStartIo entries**. Its StorNVMe hardware-extension flags are **0x01000000**. Disassembly establishes bit 24 as set by paths detecting an all-ones read of the controller's ASQ register; bit 0 is clear, which causes build-I/O to reject requests with NoDevice before NVMe command submission. This independently localizes the failure to C:'s controller/access path, beyond the generic in-page exception. Controller-register MMIO memory and the earlier onset history are absent; the logs contain a terminal failure storm, not the initiating transition.

**Additional confirmed failed write:** A captured System hard-error packet contains `c0000222 STATUS_LOST_WRITEBEHIND_DATA` for `C:\Windows\System32\winevt\Logs\Microsoft-Windows-Hyper-V-VmSwitch-Operational.evtx`. That identifies another affected C: file, not Hyper-V as the initiator.

**Supported inference:** C:'s NVMe driver detected loss of usable controller-register access. The root distinction among SSD-controller, PCIe/platform/power, firmware and software/state-corruption causes remains unresolved. The evidence does not prove defective SSD media or establish a guaranteed replacement/setting remedy. The shorter-quarter result supports investigating game activity while contradicting a required eight-minute runtime threshold; it did not reproduce the exact first-inbound moment.

**Current user constraint:** Bryan explicitly requests **no further crash-test exchanges**. The earlier reproduction proposals are superseded. No additional crash was induced or requested, and no system/game setting was changed by the assistant. Official game, Samsung, ASUS and Microsoft release notes were checked; no matching definite remedy was verified. See [complete third-dump findings and limits](native-crash-20260912-03/FINDINGS.md) and its linked storage/code evidence.
