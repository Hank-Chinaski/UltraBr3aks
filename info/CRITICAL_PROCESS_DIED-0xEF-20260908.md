...
```powershell
Get-Item -LiteralPath "E:\CrashDumps\MEMORY.DMP" -Force | Select-Object FullName,Length,LastWriteTime
```

4. Send back the command's output and the approximate local time of this crash. If the file is not found, report that exact result instead.
